...
public class ShiroRealm extends AuthorizingRealm {
    ...
    public LoginUser checkUserTokenIsEffect(String token) throws AuthenticationException {
        ...
        LoginUser loginUser = TokenUtil.getLoginUser(username, commonService, redisUtil);
        if (loginUser == null) {
            throw new AuthenticationException("用户不存在!");
        }
        // 判断用户状态-锁定
        if (CommonConstant.SYS_USER_STATUS_LOCKED.equals(loginUser.getStatus())) {
            throw new AuthenticationException("账号已被锁定,请联系管理员!");
        }
        // 判断用户状态-禁用
        if (CommonConstant.SYS_USER_STATUS_DISABLE.equals(loginUser.getStatus())) {
            throw new AuthenticationException("账号已被禁用,请联系管理员!");
        }
        // 校验token是否超时失效 & 或者账号密码是否错误
        if (!jwtTokenRefresh(token, username, loginUser.getPassword())) {
            throw new AuthenticationException(CommonConstant.MSG_TOKEN_IS_INVALID);
        }
        return loginUser;
    }
